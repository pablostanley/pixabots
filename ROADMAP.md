...
- [x] **Home refresh appears to shuffle hue + bg.** Root cause: `/` with no `?id=` param picked a fresh `randomCombo()` on every mount and never synced the URL, so refresh re-rolled the selection. Each combo has its own intrinsic pixel palette → felt like hue/bg changed. Fix: on initial mount, if no `initialId`, `history.replaceState` writes `?id=X&hue=…&saturate=…&bg=…` so refresh reproduces the same pixabot. Also threaded `bg` through the URL so palette+bg round-trip (previously only `hue`+`saturate` did).
