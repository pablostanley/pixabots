...
- [x] **Animated `cheeky-terminal` (16-frame sequence) + `terminal-round` (blink).** Upgraded two previously-static eye parts to animated. `cheeky-terminal` is the first 16-frame sequence (fills one full super-loop). `ANIM_VERSION` 1→2 to bust CDN caches. Parts.ts docstring updated to include N=16 as valid sequence length.
...
- [x] **Phase 1 — docs recipe (`/docs/creator`).** Shipped; MDX page with a ~60-LOC MiniCreator + Fx filter snippet + links to the prod files. Sitemap + docs sidebar updated.
- [x] **Phase 2 — starter example (`examples/creator-starter/`).** Shipped as an in-repo folder rather than a separate template repo. Vite + React + TypeScript + ~150 LOC App showing shuffle, cycle, URL sync, and Fx hue/saturate sliders. README explains copy-and-go. Left in-repo for now so it tracks the package version.
...
- [x] **Home refresh shuffles hue + bg.** Two root causes. First round (PR #142) assumed selection was re-rolling; fixed URL-sync on first mount so `/` → `/?id=X` for stable refresh. That *didn't* stop the hue/bg shuffle. Second round (PR #158): real culprit was ⌘R. The creator's keydown handler mapped plain `r` to `randomPalette()` but had no modifier guard — ⌘R fires `keydown "r"` before the reload, so `randomPalette` wrote random hue/saturate/bg into the URL and then the browser reloaded that URL. Fix: import `hasModifier` (already used by every other keydown site in the app) and early-return when any modifier is held. Also incidentally stops ⌘D / ⌘F / ⌘P / ⌘X / ⌘U / ⌘C from hijacking browser shortcuts.
