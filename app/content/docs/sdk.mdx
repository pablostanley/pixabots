...
Some parts ship as horizontal sprite sheets (`frames × 32` wide) so they can animate inside the 8-tick idle bounce. Each `PartOption` declares a `kind`:

- `static` (default) — always frame 0.
- `blink` — 2-frame sheet `[open, closed]`. Runtime schedule: `[0,1,0,1,0,0,0,0]` (two blinks then hold-open).
- `sequence` — N-frame sheet played in order. N should divide 8 (1 / 2 / 4 / 8) so the sub-loop fits the bounce.

Resolve a tick → frame index via `resolveFrameIndex`:

```typescript
import { PARTS, resolveFrameIndex, BLINK_SCHEDULE } from "@pixabots/core";
...
BLINK_SCHEDULE; // [0,1,0,1,0,0,0,0]
```
