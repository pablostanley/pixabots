...
| `size` | query | `128` | Image size in pixels. Any multiple of 32 between 32 and 1920 (60 sizes). Nearest-neighbor scaled from the 32×32 source. |
...
Any multiple of 32 from 32 to 1920 works — that's **60 discrete sizes**. Common choices:
...
Add `?animated=true` to get the idle bounce animation. **All 60 sizes** are supported — including 1920px:
